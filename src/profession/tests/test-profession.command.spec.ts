...
describe("Test Profession Commands", () => {
  ...
  beforeEach(async () => {
    ...
    createProfessionCommand = moduleRef.get<CreateProfessionCommand>(CreateProfessionCommand)
    getProfessionByTitleCommand = moduleRef.get<GetProfessionByTitleCommand>(GetProfessionByTitleCommand)
    getProfessionByIdCommand = moduleRef.get<GetProfessionByIdCommand>(GetProfessionByIdCommand)
    updateProfessionCommand = moduleRef.get<UpdateProfessionCommand>(UpdateProfessionCommand)
    deleteProfesionCommand = moduleRef.get<DeleteProfessionCommand>(DeleteProfessionCommand)
  ...
  describe("GET PROFESSION BY ID", () => {
    it("Should return profession by ID", async () => {
      ...
      const result = await getProfessionByIdCommand.execute("779a0f1b-ac97-4a4b-9f21-790b21580286");
  
      expect(professionRepositoryMock.findOne).toHaveBeenCalledWith({ where: { id }});
    ...
    it("Should return empty array if no profession match by ID", async () => {
      ...
      expect(getProfessionByIdCommand.execute(id)).rejects.toThrowError("Something went wrong");
  ...
  describe("GET PROFESSION BY TITLE", () => {
    it("Should return profession by same title", async () => {
      ...
      jest.spyOn(professionRepositoryMock, "find").mockResolvedValue(professions);
      const result = await getProfessionByTitleCommand.execute(profTitle);
  
      expect(professionRepositoryMock.find).toHaveBeenCalledWith({ where: { title: Like(`%${profTitle}%`) }})
    ...
    it("Should return ampty array if no profession match by title", async () => {
      ...
      jest.spyOn(professionRepositoryMock, "find").mockResolvedValue(emptyResult);
      const result = await getProfessionByTitleCommand.execute(profTitle);
  ...
  describe("CREATE COMMAND", () => {
    it("Should create profession", async () => {
      ...
      expect(professionRepositoryMock.create).toHaveBeenCalledWith(professionDto)
  ...
  describe("UPDATE COMMAND", () => {
    it("Should update profession by ID", async () => {
      ...
      jest.spyOn(professionRepositoryMock, "findOne").mockResolvedValue(profession);
      jest.spyOn(professionRepositoryMock, "update").mockResolvedValue({ affected: 1 });
      const result = await updateProfessionCommand.execute(id, updatedProfessionDto);

      expect(professionRepositoryMock.findOne).toHaveBeenCalledWith({ where: { id } });
      expect(professionRepositoryMock.update).toHaveBeenCalledWith(id, profession);
    ...
    it("Should return Bad Request Exception", async () => { 
      ...
      await expect(updateProfessionCommand.execute(fakeId, updatedProfessionDto)).rejects.toThrowError("Something went wrong with update profession")
      await expect(updateProfessionCommand.execute(id, null)).rejects.toThrowError("Something went wrong with update profession")
  ...
  describe("DELETE COMMAND", () => { 
    it("Should delete profession by ID", async () => {
      ...
      jest.spyOn(professionRepositoryMock, "delete").mockResolvedValue({ affected: 1 });
      jest.spyOn(professionRepositoryMock, "findOne").mockResolvedValue(profession);
    ...
    it("Should throw BadRequest exception", async () => {
      ...
      jest.spyOn(professionRepositoryMock, "findOne").mockResolvedValue(null);
      expect(deleteProfesionCommand.execute(id)).rejects.toThrowError("This profession is not exist");
